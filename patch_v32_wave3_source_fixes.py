...
MARKER = "WAVE3_SOURCE_FIX_V32 = True"

if MARKER in text:
    print("V32 Wave 3 source fixes already applied")
    raise SystemExit(0)
...
def replace_once(old, new, label):
    global text
    if old not in text:
        raise RuntimeError(f"V32 patch failed: marker not found for {label}")
...
# ---------------------------------------------------------------------------
# STAALZ: headless Shopify. The public staalz.dk frontend does not expose the
# normal /products.json endpoint, so discover the Shopify backend/runtime from
# the public frontend and use Storefront API (or backend Ajax JSON fallback).
# ---------------------------------------------------------------------------
staalz_code = r'''
...
def _staalz_runtime_candidates():
    ...
    # Headless storefront config normally lives in one of the frontend JS
    # bundles. Limit discovery traffic and never log/store access tokens.
...
def _staalz_storefront_raw_products(domain, token):
    endpoint = (
        f"https://{domain}/api/{STAALZ_STOREFRONT_API_VERSION}/graphql.json"
    )
    query = r'''
...
def _staalz_raw_products():
    ...
    # Preferred path for the headless Shopify storefront.
    for domain in runtime.get("domains") or []:
        for token in runtime.get("tokens") or []:
            try:
                ...
                if products:
                    print(
                        f"STAALZ: headless Shopify Storefront API fundet "
                        f"({len(products)} rå produkter)"
                    )
                    return products
            except Exception as error:
                errors.append(f"storefront {domain}: {error}")

    # Some Shopify shops still expose the Ajax product JSON on the backend
    # myshopify.com domain even when the public headless frontend does not.
    for domain in runtime.get("domains") or []:
        try:
            products = fetch_shopify_feed(
                f"https://{domain}",
                "/products.json",
            )
            if products:
                print(
                    f"STAALZ: Shopify backend Ajax feed fundet "
                    f"({len(products)} rå produkter)"
                )
                return products
        except Exception as error:
    ...
    # Cheap public-domain probes in case STAALZ later exposes a standard feed.
    for path in (
        ...
        try:
            ...
            if products:
                print(
                    f"STAALZ: public Shopify feed {path} fundet "
                    f"({len(products)} rå produkter)"
                )
                return products
        except Exception as error:
    ...
    raise RuntimeError(
        "Staalz headless Shopify kunne ikke resolveres fra offentlig frontend: "
        + details
    )
...
# ---------------------------------------------------------------------------
# KOCARDZ: Store API search returned 0, while the public WooCommerce category
# pages visibly contain the full catalog. Parse those category cards directly.
# ---------------------------------------------------------------------------
kocardz_code = r'''
...
print("Applied V32: Staalz headless Shopify + KoCardz category parser fixes")
