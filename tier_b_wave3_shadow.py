#!/usr/bin/env python3
"""Run Tier B Wave 3 as live Restock sources.
...
Price Watch remains off for Wave 3.
"""
...
from tier_b_wave3_sources import WAVE3_SOURCES, fetch_wave3_source


STATE_FILE = Path("tier_b_wave3_shadow_state.json")
STATE_VERSION = 2
MAX_DROP_RATIO = 0.70
WEBHOOK_URL = os.getenv("DISCORD_WEBHOOK_URL", "").strip()
LIVE_SOURCES = tuple(WAVE3_SOURCES)
...
def _emit_live_alerts(source_key, label, old_products, products, *, sender=_post_discord):
    ...
    if not old_products:
        print(f"WAVE3 LIVE {label}: ingen tidligere baseline; alerts undertrykt denne kørsel")
        return 0
...
def run_scan(fetcher=fetch_wave3_source, sender=None):
    # Deterministic tests inject a fetcher; keep those offline unless a sender
    # is explicitly supplied.
    if sender is None:
        sender = _post_discord if fetcher is fetch_wave3_source else _noop_sender
    ...
    for source_key, config in WAVE3_SOURCES.items():
        ...
        try:
            ...
            print(
                f"WAVE3 LIVE {config['label']}: {pokemon} Pokémon | {lorcana} Lorcana | "
            ...
            print(
                f"WAVE3 LIVE {config['label']} FEJL: {error} | "
    ...
    print(
        f"WAVE3: {len(WAVE3_SOURCES) - failures}/{len(WAVE3_SOURCES)} kilder ok | "
...
def main():
    # Individual Wave 3 failures preserve the old snapshot and must not block
