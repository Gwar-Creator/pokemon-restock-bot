#!/usr/bin/env python3
"""Scan Tier B Wave 3 without Discord or Price Watch side effects."""
...
def run_scan(fetcher=fetch_wave3_source):
    ...
    for source_key, config in WAVE3_SOURCES.items():
        ...
        try:
            ...
            print(
                f"WAVE3 SHADOW {config['label']}: {pokemon} Pokémon | {lorcana} Lorcana | "
            ...
            print(
                f"WAVE3 SHADOW {config['label']} FEJL: {error} | "
    ...
    print(
        f"WAVE3 SHADOW: {len(WAVE3_SOURCES) - failures}/{len(WAVE3_SOURCES)} kilder ok | "
