#!/usr/bin/env python3
"""V55 exact-collection + canonical-rarity shadow runner."""
...
from personal import singles_v55 as radar
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--rarity-metadata", type=Path, default=radar.DEFAULT_RARITY_METADATA)
...
def main() -> int:
    ...
    rarity_metadata = radar.load_rarity_metadata(args.rarity_metadata)
    ...
    rows = radar.evaluate_state(state, effective_profile, rarity_metadata)
    if not rows:
    ...
    header = cr.collection_header(stats, diagnostics, unresolved).replace(
        ...
        "V55 exact-link + canonical-rarity shadow",
    )
